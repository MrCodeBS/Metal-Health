...
async function generateClinicalNote(
  ...
  const conversationSummary = userMessages
    ...
    .map(
      (m, i) =>
        `[${i + 1}] User: ${m.content.substring(0, 150)}${
  ...
  if (analysis.severity === "urgent") {
    recommendations.push(
      "IMMEDIATE INTERVENTION REQUIRED - Crisis risk identified"
    );
    ...
    recommendations.push(
      "Consider referral to specialist or intensive therapy"
    );
    ...
    recommendations.push(
      "Routine monitoring - patient engaged with self-care tools"
    );
  ...
  if (userContext.moodData) {
    ...
    summary += `- Average Mood: ${userContext.moodData.averageMood.toFixed(
      1
    )}/10\n`;
    summary += `- Average Stress: ${userContext.moodData.averageStress.toFixed(
      1
    )}/10\n`;
    summary += `- Trend: ${trend}\n\n`;
  }
  ...
  if (userContext.personalityTraits) {
    ...
    for (const [trait, score] of Object.entries(
  ...
  if (analysis.concerningPatterns.length > 0) {
    ...
    analysis.concerningPatterns.forEach((p) => {
      summary += `- ${p.pattern} (${(p.confidence * 100).toFixed(
        0
      )}% confidence): ${p.evidence}\n`;
    });
  }
  ...
  await note.save();
  console.log(
    `Clinical note created for user ${userId} - Severity: ${analysis.severity}`
  );
